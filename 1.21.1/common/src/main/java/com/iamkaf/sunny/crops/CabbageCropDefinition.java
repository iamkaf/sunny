...
public final class CabbageCropDefinition extends SunnyCropDefinition {
    private static final String ID = "cabbage";
    private static final String DISPLAY_NAME = "Cabbage";
    private static final int NUTRITION = 4;
    private static final float SATURATION = 0.2f;
    private static final CropShape SHAPE = CropShape.CROP;

    private static final RegistrySupplier<Block> CROP_BLOCK = Blocks.BLOCKS.register(ID + "_crop_block",
            () -> new GenericCropBlock(BlockBehaviour.Properties.ofFullCopy(net.minecraft.world.level.block.Blocks.WHEAT), SHAPE) {
                @Override
                protected @NotNull ItemLike getBaseSeedId() {
                    return CROP_ITEM.get();
                }
            }
    );

    private static final RegistrySupplier<Item> CROP_ITEM = Items.ITEMS.register(ID,
            () -> new ItemNameBlockItem(CROP_BLOCK.get(), new Item.Properties().food(new FoodProperties.Builder().nutrition(NUTRITION)
                    .saturationModifier(SATURATION)
                    .build()).arch$tab(CreativeModeTabs.SUNNY))
    );


    public CabbageCropDefinition() {
